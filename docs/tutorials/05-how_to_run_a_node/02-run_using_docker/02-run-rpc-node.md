...
In order to submit extrinsics (transactions, in Substrate terminology) with PolkadotJS from one of your accounts, you need to import it within the application.

Navigate to the `Settings` section, then to the subsection `account options`, open the drop-down list `in-browser account creation`, select the option `Allow local in-browser account storage` and click on the `Save` button:

![alt_text](./img/polkadotjs_settings.png)

Next, navigate to the section `Accounts` then to the subsection `Accounts` and click on the `Account` button:

![alt_text](./img/polkadotjs_accounts.png)

Fill in the secret phrase or raw seed associated with your account (ignore the newly one generated by the wizard), tick the checkbox `I have saved my mnemonic seed safely` and click on the `Next` button:

![alt_text](./img/polkadotjs_accounts_s1.png)

Then provide inputs for fields `name`, `password`, `password (repeat)` and click on the `Next` button:

![alt_text](./img/polkadotjs_accounts_s2.png)

Finally, review the summary and click on the confirmation button `Save`:

![alt_text](./img/polkadotjs_accounts_s3.png)

After importing your account, you can send funds from the same page by clicking on the `send` button:
...
In a few seconds, you should receive feedback in a popup message on the top-right corner confirming that the extrinsic has been successfully submitted and the funds have been transferred.
